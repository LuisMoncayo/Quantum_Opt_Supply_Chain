...
# -*- coding: utf-8 -*-
"""
Line Balancing QAOA — Unbalanced Penalisation (Statistical Analysis, Noisy)
Simulates QAOA with the hardware-calibrated noise model from 'ibm_brisbane'
...
N_RUNS = 30
λ1 = 1
λ2 = 12             # ← fixed penalty parameters
reps = 5
shots = 8000
...
df = pd.read_excel(path, sheet_name="LB_8", header=None)

nu_variables, nu_constraints = 24,20
...
# ============================================================
# 6. Simulator setup — with hardware noise
# ============================================================
print("\n🔧 Loading IBM backend noise model: ibm_brisbane")

service = QiskitRuntimeService()
backend_real = service.backend("ibm_torino")

# Build noisy simulator automatically (handles delays)
backend = AerSimulator.from_backend(backend_real, method="density_matrix")

estimator = Estimator(run_options={"backend": backend})
sampler = Sampler(run_options={"backend": backend})
print("✅ Noisy AerSimulator initialised (ibm_brisbane, density_matrix).")
...
# Create a transpiled version for the noisy backend
qc_transpiled = transpile(ansatz, backend, optimization_level=1)
...
for run in range(1, N_RUNS + 1):
    ...
    shot_records = []
    for bitstring, prob in counts.items():
        n_occurrences = int(prob * shots)
        for s_id in range(n_occurrences):
            shot_records.append({
                "run": run,
                "shot_id": s_id + 1,
                "bitstring": "'" + bitstring,
                "probability": prob,
                "sum_x": np.sum(list(map(int, bitstring[::-1]))),
                "feasible": bool(df_r.loc[df_r["bitstring"] == "'" + bitstring, "feasible"].values[0])
            })
    ...
    results.append((run, best_fun, feas_mass, n_feas, start_dt, end_dt, elapsed))
...
df_runs = pd.DataFrame(
    results,
    columns=["run", "best_energy", "feas_mass", "n_feas", "start_time", "end_time", "elapsed_sec"]
)
df_runs["energy_norm"] = df_runs["best_energy"] / λ2
summary = df_runs.describe().T

out_summary = f"LB5_QAOA_Brisbane_Noise_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
